...
⚠️ **데이터 시차 유의사항**:
아래 제공되는 데이터는 **전일(어제) 장 마감 후** 수집된 과거 데이터입니다.
당신의 임무는 이 과거 데이터를 기반으로 **오늘(장 시작 이후) 각 종목의 시그널 향방을 예측**하는 것입니다.
따라서 분석 시 다음을 반드시 고려하세요:
- 어제까지의 추세, 수급, 거래량 흐름이 오늘 장에서 어떻게 이어질지 전망
- 외국인/기관의 연속 매매 패턴이 오늘도 지속될 가능성 평가
- 뉴스/재료의 시장 반영 여부 (이미 반영된 재료 vs 오늘 추가 반영될 재료)
- 시그널은 "어제 기준 현황"이 아닌 "오늘 기준 예측"으로 도출하세요
...
### 2-3. 밸류에이션 분석 (가중치 20%)
각 항목을 0~10점으로 채점한 뒤 평균을 산출하세요:
- **PER 절대 수준**: PER<10=8~10, 10~15=6~7, 15~25=4~5, >25=1~3, 적자(음수)=2
  ⚠️ 단, 바이오/IT/성장주는 PER>25가 정상일 수 있음. 업종 특성상 고PER이 합리적이면 PEG나 목표주가 괴리율로 보완 판단.
- **PBR 절대 수준**: PBR<0.8=8~10, 0.8~1.5=5~7, 1.5~3.0=3~5, >3.0=1~3
- **PEG**: <1=8~10, 1~2=4~6, >2=0~3 (추정EPS 가용 시)
- **목표주가 괴리율**: 현재가가 목표가 대비 20%↓=8~10, 근접=4~6, 20%↑=0~3
- **데이터 미확인 시**: 해당 항목을 채점에서 제외하고 나머지 항목으로 평균 산출. ⚠️ 5.0을 기본값으로 사용하지 마세요.
...
⚠️ 채점 보정 규칙:
- **뉴스 없음 또는 일반적 전망 기사만 있음** → 5.0 (중립)
- **구체적 수치/이벤트가 포함된 호재** (실적 서프라이즈, 대형 계약, 상향 조정 등) → 7~10
- **구체적 수치/이벤트가 포함된 악재** (실적 미스, 소송, 하향 조정 등) → 0~3
- "전망 밝다", "기대감" 같은 모호한 긍정 표현만 있으면 → 5~6 (중립~약간 긍정)
- 1주일 이상 된 뉴스만 있으면 → 4~5 (시의성 감점)
- ⚠️ **장 시작 전 분석 시**: 당일 뉴스가 아직 생성되지 않았을 수 있습니다. 이 경우 전일 장 마감 이후 뉴스를 기준으로 판단하세요. 검색 결과가 적다고 해서 재료 점수를 낮추지 마세요. 뉴스 부재 자체는 악재가 아닙니다.
...
1. **점수 분포 의식**: 채점 후 결과를 돌아보세요. 만약 특정 세부 점수(예: valuation)가 대부분의 종목에서 4~6점에 몰려있다면, 채점 기준을 충분히 활용하지 않은 것입니다. 실제 데이터의 차이를 점수에 반영하세요. 약세장에서 대부분이 낮은 점수를 받는 것은 정상이며, 인위적으로 분포를 맞출 필요는 없습니다.

2. **5.0 사용 제한**: 어떤 세부 점수든 5.0을 부여하려면 구체적 근거가 있어야 합니다. "데이터 부족"이나 "판단 불가"는 5.0의 사유가 될 수 없습니다. → 데이터가 없으면 해당 하위 항목을 제외하고 나머지로 평균.

3. **극단값 허용**: 매우 강한 매수/매도 신호가 있으면 9~10점 또는 0~1점을 주저하지 마세요.
...
### 2-1. 기술적 분석 (가중치 30%)
각 항목을 0~10점으로 채점한 뒤 평균을 산출하세요:
- **추세** (ma_status 기준): bullish + 20일 이격도 양수=9, bullish=8, mixed + 상승 전환 징후=6, mixed=5, bearish + 하락 가속=1, bearish=3
- **52주 고저 위치**: ((price - low_52w) / (high_52w - low_52w) × 100)으로 계산. 0~15%=9, 15~30%=7, 30~50%=5, 50~70%=4, 70~85%=3, 85~100%=1
- **거래량**: vol_rate 300↑=9, 200~300=8, 150~200=7, 100~150=5, 70~100=4, 50~70=3, 50↓=1
- **RSI-14**: 30~40(과매도 반등 구간)=9, 40~50=7, 50~60=5, 60~70=4, 70~80=2, 80↑=1, 25↓=3(급락 주의)

### 2-2. 수급 분석 (가중치 35%)
각 항목을 0~10점으로 채점한 뒤 평균을 산출하세요:
- **외국인 동향** (for_consec_buy 기준): 5일 연속 순매수=9, 4일=8, 3일=7, 2일=6, 매수/매도 교차=5, 2일 연속 순매도=4, 3일=3, 4일=2, 5일=1. 추가로 for_20d(20일 누적)가 양수이면 +0.5 보정.
- **기관 동향** (inst_consec_buy 기준): 외국인과 동일 기준 적용.
- **호가 잔량** (bid_ask_ratio): >200=9, 150~200=7, 100~150=5, 70~100=4, <70=2. ⚠️ 장 마감 후 데이터는 다음 날 시초 호가를 반영하므로, 극단값(>300 또는 <30)이 아닌 한 과도한 가중을 두지 마세요.

### 2-3. 밸류에이션 분석 (가중치 20%)
각 항목을 0~10점으로 채점한 뒤 평균을 산출하세요:
- **PER 절대 수준**: PER<8=9, 8~10=8, 10~13=7, 13~15=6, 15~20=5, 20~25=4, 25~35=3, 35~50=2, >50=1, 적자(음수)=2
  ⚠️ 단, 바이오/IT/플랫폼 등 성장주는 PER>25가 정상일 수 있음. 업종 특성상 고PER이 합리적이면 PEG나 EPS 성장률로 보완 판단.
- **PEG**: <0.5=9, 0.5~1.0=8, 1.0~1.5=6, 1.5~2.0=4, >2.0=2 (null이면 생략)
- **ROE**: >20%=9, 15~20%=8, 10~15%=6, 5~10%=4, <5%=2 (fundamental 필드 있을 때)
- **부채비율**: <50%=9, 50~100%=7, 100~150%=5, 150~200%=3, >200%=1
- **EPS 성장률**: >30%=9, 20~30%=8, 10~20%=6, 0~10%=4, 음수=2
- **영업이익률**: >20%=9, 15~20%=8, 10~15%=6, 5~10%=4, <5%=2
- 데이터가 null인 항목은 채점에서 제외. 나머지 항목으로 평균 산출. ⚠️ 데이터 없음 = 5.0이 아님. 채점 대상에서 완전히 제외하세요.
...
4. **기술적 분석 점수 분포 검증**: 기술적 분석(technical) 세부 점수가 대부분 4~6점에 몰려있다면, ma_status, vol_rate, rsi_14 각 필드의 실제 수치를 다시 확인하세요. 예: ma_status="bullish"인데 technical=5점은 과소평가입니다. 데이터가 명확한 방향성을 보이면 7점 이상 또는 3점 이하를 부여하세요.

...
- 테마 및 섹터 모멘텀 평가
- 뉴스 시의성: 오늘 날짜 기준 1주일 이내를 '최근'으로 간주하세요.
- 뉴스가 없는 종목: google_search 도구로 "{{종목명}} 주식 뉴스"를 검색하여 news_analysis를 작성하세요. 검색 결과도 없으면 sentiment="중립", catalyst="관련 뉴스 없음"으로 설정하세요.
...
5. 뉴스가 없는 종목은 google_search로 검색 후 news_analysis를 작성하세요. 검색 결과도 없으면 sentiment="중립", catalyst="관련 뉴스 없음"으로 설정하세요.
"""
        ...
        try:
            ...
            response = client.models.generate_content(
                ...
                config={
                    ...
                    "tools": [{"google_search": {}}],  # 뉴스 없는 종목용 실시간 검색
                }
            )

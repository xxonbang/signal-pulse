...
export function MarketTabs({ active, counts, onChange }: MarketTabsProps) {
  return (
    <div className="flex gap-1 md:gap-2 mb-4 bg-bg-secondary p-1 md:p-1.5 rounded-xl border border-border" role="tablist" aria-label="시장 구분">
      {tabs.map((tab) => (
        <button
          key={tab.key}
          role="tab"
          ...
          className={cn(
            'flex-1 py-2.5 md:py-3 px-2 md:px-4 rounded-lg text-xs md:text-sm font-semibold transition-colors flex items-center justify-center gap-1 md:gap-2',
            active === tab.key
